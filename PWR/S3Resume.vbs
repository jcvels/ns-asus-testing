...
Const PERF_PWR_TEST_COMMAND = ".\PwrTest.exe /sleep /c:4 /d:90 /p:90 /h:n /s:3 /IgnoreUsbHub /qpccheck /ln:pwrtestlog"
...
Const SYSTEMINFO_NODE_NAME  = "SystemInformation"
Const SYSTEMINFO_XML_PACKAGECOUNT_NAME = "ProcessorPackageCount"
...
Sub PerfParseXmlFile
   ...
   ' Note: even though the test type actual parameter is 
   ' WTTLOG_TESTCASE_TYPE_NOPARENT, we still have to pass in 
   ' GUID and test name for 4th and 5th arguments to work 
   ' around a WTT logger issue
   g_WTTLog.StartTestEx GUIDOfS3Resume, S3LogTestName, WTTLOG_TESTCASE_TYPE_NOPARENT, GUIDOfS3Resume, S3LogTestName , g_WTTHandle
   ...
   If(1=g_bLogPass)  Then
      ...
      '
      ' Determine whether this system is exempt from resume time requirements.
      '

      If(PerfGetPackageCount() > 1) Then
          WTTResult = WTT_TC_RESULT_PASS
          PerfLogToPerfFileAndDisplay ("This system is exempt from resume requirements because it contains more than one processor package")
          g_WTTLog.EndTestEx WTTResult, GUIDOfS3Resume, S3LogTestName, GUIDOfS3Resume, S3LogTestName, g_WTTHandle
...
Function PerfGetPackageCount()

'
' Selects the ProcessorPackageCount node and returns an integer representing
' the number of processor packages on the system.
'
' Logs a message indicating the number of processor packages.
'

   Dim EventNodeList
   Set EventNodeList = g_S3SleepPerfTraceFileDOM.SelectNodes(PERF_XML_NODE_LIST_SEARCH_PREFIX & SYSTEMINFO_NODE_NAME)

   If (EventNodeList.length <> 1) Then
       PerfGetPackageCount = 0
       Exit Function
   End If

   Dim EventNode
   Dim ProcessorPackageCount
   Dim xEventNode

   xEventNode = 0
   Set EventNode = EventNodeList.Item (xEventNode)

   PerfGetTextForId EventNode, SYSTEMINFO_XML_PACKAGECOUNT_NAME, ProcessorPackageCount
   PerfLogToPerfFileAndDisplay ("ProcessorPackageCount=" & ProcessorPackageCount)
   PerfGetPackageCount = CInt (ProcessorPackageCount)
End Function
...
Sub PerfDisplayUsage 
   ...
   WScript.Echo("        2--Parse PwrTestlog.xml ")
   WScript.Echo(" ")
...
'   TotalResumeTime = DriverInitTime + BiosInitTime
'
...
Sub PerfOpenAdvancedPerformanceTraceFileWithDOM (ByRef PerfLogFileXML, PerfLogFileName)
   ...
   If (PerfLogFileXML.parseError.errorCode <> 0) Then
      ...
      PerfLogToPerfFileAndDisplay("*** PwrTest Log File Error Detected ***")
      PerfLogToPerfFileAndDisplay(" ")
